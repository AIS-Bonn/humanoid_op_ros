...
#include <rc_utils/attitude_estimator.h> // Configured by ATT_EST_PRIVATE_ARE_AVAILABLE
...
{
	// Declare variables
	double q1[4] = {0.0}, q2[4] = {0.0}, q3[4] = {0.0};
	...
	EXPECT_DOUBLE_EQ(10.0, KpQ);
	EXPECT_DOUBLE_EQ(1.25, TiQ);
	Est.setPIGains(-1e8, 7.40, 8.00, 3.00);
	Est.getPIGains(Kp, Ti, KpQ, TiQ);
	EXPECT_DOUBLE_EQ(4.00, Kp);
	EXPECT_DOUBLE_EQ(3.00, Ti);
	...
	// Test getting the attitude estimate with a specified offset
	q2[0] = 7.8;
	q2[1] = -5.6;
	q2[2] = 3.4;
	q2[3] = -2.1;
	...
	q3[2] = 0.0;
	q3[3] = 0.0;
	Est.getAttitude(q1, q3);
	EXPECT_DOUBLE_EQ(q2[0], q1[0]);
	EXPECT_DOUBLE_EQ(q2[1], q1[1]);
	EXPECT_DOUBLE_EQ(q2[2], q1[2]);
	EXPECT_DOUBLE_EQ(q2[3], q1[3]);
	q3[0] = 0.0;
	q3[1] = 0.0;
	q3[2] = 0.0;
	q3[3] = 0.0;
	...
	q3[3] = 5.6;
	Est.getAttitude(q1, q3);
	EXPECT_DOUBLE_EQ(-0.047246457571266054, q1[0]);
	EXPECT_DOUBLE_EQ(-0.065807565902834819, q1[1]);
	EXPECT_DOUBLE_EQ(-0.996300299736935235, q1[2]);
	EXPECT_DOUBLE_EQ( 0.028685349239697253, q1[3]);
