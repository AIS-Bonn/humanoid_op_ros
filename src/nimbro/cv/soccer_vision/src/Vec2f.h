...
//      inline float& operator[](int i){assert((0<=i)&&(i<2));	return (&x)[i];};
		...
		inline float norm() const { return sqrtf(x*x+y*y);} // Euklidische Länge des Vektors zum Quadrat
		/* Euklidischer Länge eines Vektors zum Quadrat */
		inline float norm2() const { return x*x+y*y;}  // Euklidische Länge des Vektors zum Quadrat
